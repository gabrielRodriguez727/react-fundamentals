...
useRef =>  Is an object with only a current property that stays consistent between renders and changing does not trigger a re-render.
React expects that all the comoponents behaves like a pure functions:
   For the same inputs (props, state, context) must return the same output (JSX)
   Don't produce side effects
Read or write a ref during rendering breaks these expectations.
If you pass the ref to a HTML node, you can access it later by the current prop.
React custom components don't expose ref to the DOM nodes inside them, 
to fix this you need to wrap the component in a forwardRef
